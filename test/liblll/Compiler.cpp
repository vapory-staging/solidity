...
bool successCompile(string const& _sourceCode)
{
	...
	bytes bytecode = vap::compileLLL(_sourceCode, dev::test::Options::get().vvmVersion(), false, &errors);
	if (!errors.empty())
		return false;
	if (bytecode.empty())
		return false;
...
BOOST_AUTO_TEST_CASE(valid_opcodes_functional)
{
	...
	for (size_t i = 0; i < opcodes_bytecode.size(); i++) {
		vector<string> errors;
		bytes code = vap::compileLLL(opcodes_lll[i], dev::test::Options::get().vvmVersion(), false, &errors);
