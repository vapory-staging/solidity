...
#include <libvvmasm/CommonSubexpressionEliminator.h>
#include <libvvmasm/PeepholeOptimiser.h>
#include <libvvmasm/JumpdestRemover.h>
#include <libvvmasm/ControlFlowGraph.h>
#include <libvvmasm/BlockDeduplicator.h>
...
namespace
{
	...
	vap::KnownState createInitialState(AssemblyItems const& _input)
	{
		vap::KnownState state;
		for (auto const& item: addDummyLocations(_input))
			state.feedItem(item, true);
		return state;
	}

	AssemblyItems CSE(AssemblyItems const& _input, vap::KnownState const& _state = vap::KnownState())
	{
		AssemblyItems input = addDummyLocations(_input);

		bool usesMsize = (find(_input.begin(), _input.end(), AssemblyItem{Instruction::MSIZE}) != _input.end());
		vap::CommonSubexpressionEliminator cse(_state);
	...
	void checkCSE(
		...
		KnownState const& _state = vap::KnownState()
	)
	{
...
{
	vap::KnownState state;
	vap::CommonSubexpressionEliminator cse(state);
	AssemblyItems input{
...
{
	vap::KnownState state = createInitialState(AssemblyItems{
		u256(0x12),
		u256(0x20),
		Instruction::ADD
	});
...
{
	vap::KnownState state = createInitialState(AssemblyItems{Instruction::DUP1});
	AssemblyItems input{
		Instruction::EQ
	};
...
BOOST_AUTO_TEST_CASE(jumpdest_removal_subassemblies)
{
	...
	main.optimise(true, dev::test::Options::get().vvmVersion());
