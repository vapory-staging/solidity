...
Json::Value StandardCompiler::compileInternal(Json::Value const& _input)
{
	...
	if (settings.isMember("vvmVersion"))
	{
		boost::optional<VVMVersion> version = VVMVersion::fromString(settings.get("vvmVersion", {}).asString());
		if (!version)
			return formatFatalError("JSONError", "Invalid VVM version requested.");
		m_compilerStack.setVVMVersion(*version);
	}
