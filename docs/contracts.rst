...
.. note::
  ...
  prevent modifications to the state on the level of the VVM by adding
...
.. warning::
  ...
  of the VVM, it is only possible to prevent them from writing to the state
  (i.e. only ``view`` can be enforced at the VVM level, ``pure`` can not).
...
The VVM does not provide a direct way for a contract to detect
