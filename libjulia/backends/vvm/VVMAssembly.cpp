...
*/
/**
 * Assembly interface for VVM and VVM1.5.
 */
...
namespace
{
/// Size of labels in bytes. Four-byte labels are required by some VVM1.5 instructions.
...
void VVMAssembly::setSourceLocation(SourceLocation const&)
{
	// Ignored for now;
}

void VVMAssembly::appendInstruction(solidity::Instruction _instr)
{
	m_bytecode.push_back(byte(_instr));
	m_stackHeight += solidity::instructionInfo(_instr).ret - solidity::instructionInfo(_instr).args;
}

void VVMAssembly::appendConstant(u256 const& _constant)
{
...
void VVMAssembly::appendLabel(LabelID _labelId)
{
	setLabelToCurrentPosition(_labelId);
	appendInstruction(solidity::Instruction::JUMPDEST);
}

void VVMAssembly::appendLabelReference(LabelID _labelId)
{
	solAssert(!m_vvm15, "Cannot use plain label references in EMV1.5 mode.");
...
VVMAssembly::LabelID VVMAssembly::newLabelId()
{
	m_labelPositions[m_nextLabelId] = size_t(-1);
	return m_nextLabelId++;
}

AbstractAssembly::LabelID VVMAssembly::namedLabel(string const& _name)
{
...
void VVMAssembly::appendLinkerSymbol(string const&)
{
	solAssert(false, "Linker symbols not yet implemented.");
}

void VVMAssembly::appendJump(int _stackDiffAfter)
{
	solAssert(!m_vvm15, "Plain JUMP used for VVM 1.5");
	appendInstruction(solidity::Instruction::JUMP);
	m_stackHeight += _stackDiffAfter;
}

void VVMAssembly::appendJumpTo(LabelID _labelId, int _stackDiffAfter)
{
	if (m_vvm15)
	{
...
void VVMAssembly::appendJumpToIf(LabelID _labelId)
{
	if (m_vvm15)
	{
...
void VVMAssembly::appendBeginsub(LabelID _labelId, int _arguments)
{
	solAssert(m_vvm15, "BEGINSUB used for VVM 1.0");
...
void VVMAssembly::appendJumpsub(LabelID _labelId, int _arguments, int _returns)
{
	solAssert(m_vvm15, "JUMPSUB used for VVM 1.0");
...
void VVMAssembly::appendReturnsub(int _returns, int _stackDiffAfter)
{
	solAssert(m_vvm15, "RETURNSUB used for VVM 1.0");
	solAssert(_returns >= 0, "");
	m_bytecode.push_back(byte(solidity::Instruction::RETURNSUB));
	m_stackHeight += _stackDiffAfter - _returns;
}

vap::LinkerObject VVMAssembly::finalize()
{
	...
	vap::LinkerObject obj;
	obj.bytecode = m_bytecode;
	return obj;
}

void VVMAssembly::setLabelToCurrentPosition(LabelID _labelId)
{
...
void VVMAssembly::appendLabelReferenceInternal(LabelID _labelId)
{
	m_labelReferences[m_bytecode.size()] = _labelId;
	m_bytecode += bytes(labelReferenceSize);
}

void VVMAssembly::appendAssemblySize()
{
...
void VVMAssembly::updateReference(size_t pos, size_t size, u256 value)
{
