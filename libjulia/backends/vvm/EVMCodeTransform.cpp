...
*/
/**
 * Common code generator for translating Julia / inline assembly to VVM and VVM1.5.
 */
...
void CodeTransform::operator()(FunctionCall const& _call)
{
	...
	VVMAssembly::LabelID returnLabel(-1); // only used for vvm 1.0
	if (!m_vvm15)
	{
...
{
	solAssert(!m_vvm15 || _instruction.instruction != solidity::Instruction::JUMP, "Bare JUMP instruction used for VVM1.5");
	solAssert(!m_vvm15 || _instruction.instruction != solidity::Instruction::JUMPI, "Bare JUMPI instruction used for VVM1.5");
	m_assembly.setSourceLocation(_instruction.location);
	m_assembly.appendInstruction(_instruction.instruction);
	checkStackHeight(&_instruction);
}
...
void CodeTransform::operator()(FunctionDefinition const& _function)
{
	...
	int const localStackAdjustment = m_vvm15 ? 0 : 1;
