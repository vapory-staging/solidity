...
unsigned ConstantOptimisationMethod::optimiseConstants(
	bool _isCreation,
	size_t _runs,
	solidity::VVMVersion _vvmVersion,
	Assembly& _assembly,
...
{
	return vap::bytesRequired(_items, 3); // assume 3 byte addresses
}
...
{
	// This is a tiny VVM that can only evaluate some instructions.
	vector<u256> stack;
...
bigint ComputeMethod::gasNeeded(AssemblyItems const& _routine) const
{
	...
	return combineGas(
		simpleRunGas(_routine) + numExps * (GasCosts::expGas + GasCosts::expByteGas(m_params.vvmVersion)),
