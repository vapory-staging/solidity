...
#include <libvvmasm/AssemblyItem.h>
...
namespace GasCosts
{
	...
	inline unsigned extCodeGas(VVMVersion _vvmVersion)
	{
		return _vvmVersion >= VVMVersion::tangerineWhistle() ? 700 : 20;
	}
	inline unsigned balanceGas(VVMVersion _vvmVersion)
	{
		return _vvmVersion >= VVMVersion::tangerineWhistle() ? 400 : 20;
	}
	static unsigned const expGas = 10;
	inline unsigned expByteGas(VVMVersion _vvmVersion)
	{
		return _vvmVersion >= VVMVersion::spuriousDragon() ? 50 : 10;
	}
	static unsigned const keccak256Gas = 30;
	static unsigned const keccak256WordGas = 6;
	inline unsigned sloadGas(VVMVersion _vvmVersion)
	{
		return _vvmVersion >= VVMVersion::tangerineWhistle() ? 200 : 50;
	}
	...
	inline unsigned callGas(VVMVersion _vvmVersion)
	{
		return _vvmVersion >= VVMVersion::tangerineWhistle() ? 700 : 40;
	}
	...
	inline unsigned selfdestructGas(VVMVersion _vvmVersion)
	{
		return _vvmVersion >= VVMVersion::tangerineWhistle() ? 5000 : 0;
	}
...
public:
	...
	GasMeter(std::shared_ptr<KnownState> const& _state, solidity::VVMVersion _vvmVersion, u256 const& _largestMemoryAccess = 0):
		m_state(_state), m_vvmVersion(_vvmVersion), m_largestMemoryAccess(_largestMemoryAccess) {}
