...
GasMeter::GasConsumption GasMeter::estimateMax(AssemblyItem const& _item, bool _includeExternalCosts)
{
	...
	case Operation:
	{
		...
		case Instruction::SLOAD:
			gas = GasCosts::sloadGas(m_vvmVersion);
			break;
		...
		case Instruction::EXTCODESIZE:
			gas = GasCosts::extCodeGas(m_vvmVersion);
			break;
		...
		case Instruction::STATICCALL:
		{
			...
			{
				gas = GasCosts::callGas(m_vvmVersion);
		...
		case Instruction::SELFDESTRUCT:
			gas = GasCosts::selfdestructGas(m_vvmVersion);
			gas += GasCosts::callNewAccountGas; // We very rarely know whether the address exists.
			break;
		...
		case Instruction::EXP:
			gas = GasCosts::expGas;
			if (u256 const* value = classes.knownConstant(m_state->relativeStackElement(-1)))
				gas += GasCosts::expByteGas(m_vvmVersion) * (32 - (h256(*value).firstBitSet() / 8));
			else
				gas += GasCosts::expByteGas(m_vvmVersion) * 32;
			break;
		case Instruction::BALANCE:
			gas = GasCosts::balanceGas(m_vvmVersion);
			break;
		default:
			gas = runGas(_item.instruction());
			break;
